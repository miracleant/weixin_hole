...
if($c){
  ...
  $view='<div class="view left"> 点:'.(int)$con[$i]['click'].' 评:'.(int)$con[$i]['comc'].'</div>';
    ...
    $str .= 
      '<a class="item" style="background:'.$con[$i]['color'].';" href="article.php?id='.$con[$i]['id'].'"><img class="point" src="../img/point.png" /><div class="num none">'.($i+$first+1).'</div><div class="view left">#'.$con[$i]['lou'].'</div>'.$view.'<div class="time right"><span class="timed">'.date('d',$con[$i]['time']).'</span>日<span class="timem">'.date('m',$con[$i]['time']).'</span>月'.date('Y年 D H:i',$con[$i]['time']).'</div><h1 class="title">'.$con[$i]['title'].'</h1><div class="des feeling">'.$con[$i]['des'].'</div></a>';
    }
  }else{
    $note= '没有内容！';
}
