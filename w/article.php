...
$a='div';$view='';
    $str .= 
      '<'.$a.' class="item" style="background:'.$comment[$i]['color'].';" href="article.php?id='.$comment[$i]['id'].'"><img class="point" src="../img/point.png" /><div class="num none">'.($i+$first+1).'</div><div class="view left">#'.$comment[$i]['lou'].'</div>'.$view.'<div class="time right"><span class="timed">'.date('d',$comment[$i]['time']).'</span>日<span class="timem">'.date('m',$comment[$i]['time']).'</span>月'.date('Y年 D H:i',$comment[$i]['time']).'</div><h1 class="title">'.$comment[$i]['title'].'</h1><div class="des feeling">'.$comment[$i]['des'].'</div></'.$a.'>';
    }
  }else{
    $note= '没有评论，来一条吧～';
}
